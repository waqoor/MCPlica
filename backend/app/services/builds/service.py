...
class BuildService:
        ...
        *,
        ...
        async with self._database.session_scope() as session:
            ...
            has_documentation = any(
                item.source.kind is SourceKind.DOCUMENTATION for item in bindings
            )
            if has_documentation and not models.embedding_model:
                raise InvalidStateError(
                    "An embedding model is required when documentation is attached"
                )
            ...
            access = await self._access.get_config(session, project_id)
            access_mode = access.mode.value if access is not None else "static_bearer"
            inbound_mode = cast(
                Literal["static_bearer", "oidc", "none"],
                {
                    "static_bearer": "static_bearer",
                    "external_oauth_oidc": "oidc",
                    "disabled_dev": "none",
                }[access_mode],
            )
            build_config = BuildConfiguration(
                ...
                inbound_auth_mode=inbound_mode,
            ...
            build = await self._builds.create(
                ...
                embedding_model=models.embedding_model if has_documentation else None,
        ...
        try:
            await self._queue.enqueue_build(build_id)
            ...
            async with self._database.session_scope() as session:
                await self._builds.fail(
                    ...
                    error_summary="Build could not be enqueued",
                )
                await self._audit.append(
                    session,
                    actor_user_id=requested_by,
                    ...
                    metadata={
                        ...
                        "stage": BuildStatus.QUEUED.value,
                    },
                )
            raise
        return build
    ...
    async def cancel(
        ...
        *,
        ...
        async with self._database.session_scope() as session:
            ...
            if existing is None:
                raise NotFoundError("Build was not found")
            build = await self._builds.cancel(session, build_id)
            await self._audit.append(
                session,
    ...
    async def operations(self, build_id: UUID) -> list[BuildOperationView]:
        ...
        exclusions = {item.operation_key: item for item in config.excluded_operations}
        result: list[BuildOperationView] = []
        for operation in sorted(snapshot.canonical.operations, key=lambda item: item.key):
            tool = tools.get(operation.key)
            semantic = enrichment.operations.get(operation.key) if enrichment else None
            exclusion = exclusions.get(operation.key)
            ...
            result.append(
                BuildOperationView(
                    ...
                    title=(semantic.title if semantic and semantic.title else operation.summary),
