...
| Deployment activating     | Candidate edge identity, persisted activation phase/proof, superseded `RUNNING`/`STOPPING` rows and containers, worker logs | Retry the same deployment job; the worker must obtain a fresh exact edge proof before cleanup; do not provision another candidate or mark it `RUNNING` manually |
