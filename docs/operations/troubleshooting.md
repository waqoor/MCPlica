...
| API is healthy but not ready | Readiness dependency map | Recover PostgreSQL/Redis/artifact/queue for core readiness; Milvus/OpenRouter false means Builder degradation |
...
| Runtime route returns Traefik 404 | Deployment status, edge network, labels, exact `/readyz` host probe | Use the recorded restart/redeploy lifecycle and wait for edge proof; do not attach arbitrary containers |
