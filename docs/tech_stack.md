...
- generate TypeScript API types/client bindings from the backend OpenAPI schema during development/CI, or
- maintain a thin typed fetch client validated by Zod only where generation is unsuitable.
...
Use mypy in strict/near-strict project configuration.
...
- automated runtime protocol suite.
