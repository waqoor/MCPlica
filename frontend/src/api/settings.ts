...
export const settingsApi = {
  get: (signal?: AbortSignal) =>
    api<SystemSettings>("/api/v1/settings", { signal }),
  ...
  models: (signal?: AbortSignal) =>
    api<ModelSettings>("/api/v1/settings/models", { signal }),
  updateModels: (payload: ModelSettingsUpdate) =>
    api<ModelSettings>("/api/v1/settings/models", {
      method: "PUT",
      body: jsonBody({
  ...
  updateOpenRouter: (apiKey: string) =>
    api<ModelSettings>("/api/v1/settings/openrouter", {
      method: "PUT",
      body: jsonBody({ api_key: apiKey }),
    }),
  testOpenRouter: () =>
    api<{ ok: boolean; message: string }>("/api/v1/settings/openrouter/test", {
      method: "POST",
    }),
  modelCatalog: (signal?: AbortSignal) =>
    api<ModelCatalogItem[]>("/api/v1/settings/models/catalog", { signal }),
};

export const userApi = {
  list: (signal?: AbortSignal) => api<User[]>("/api/v1/users", { signal }),
  create: (payload: {
    email: string;
    display_name: string;
    role: "admin" | "builder";
    password: string;
  }) => api<User>("/api/v1/users", { method: "POST", body: jsonBody(payload) }),
  update: (
    userId: string,
